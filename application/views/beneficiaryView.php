				...
				<div class="row">
					<div class="col-lg-12">
						<div class="card alert">
						   <?php
						   if ($this->session->userdata("role")=="accountant"||$this->session->userdata("role")=="superadmin") {
						   	?>
							   <a href="<?=base_url() ?>Admin/beneficiaryAddView">
							   	<button type="button" class="btn btn-primary btn-flat btn-addon m-b-10 m-l-5">
									   <i class="ti-plus"></i>New beneficiary</button>
							   </a>
						   	<?php
						   }
						   ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#dataTable_gs').DataTable({
			...
			'serverMethod': 'post',
			'ajax': {
				'url':'<?=base_url()?>admin/getVendorList'
			},
			"columnDefs": [
				{
